...
- PR #6, `perf/finite-fluid-budget`, is stacked on PR #4 and adds deterministic global/per-chunk finite-fluid neighbour-scan active-work budgeting, thermal-aware quiet-state caching, and diagnostics for budget claims, deferrals, quiet-cache hits, thermal-cache skips, and water/lava chunk hotspots. The current server-friendly default is `256` active cells per tick and `64` active cells per chunk.
