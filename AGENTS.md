...
- Run `scripts/dev_smoke.ps1` for build, config/resource hygiene, jar inspection, server GameTests, and optional Prism copy. It keeps full Gradle logs under `build/reports/emergent-smoke` and prints compact failure hints, so inspect the log path before rerunning noisy commands.
